...
print_bold_with_outline "Welcome to ir V4, user!"
...
echo -e "${BLUE}[1/4] Cloning ir Unblocker repository...${RESET}"
if git clone https://github.com/irNetwork/v4 > /dev/null 2>&1; then
...
echo -e "${BLUE}[3/4] Starting ir Unblocker...${RESET}"
npm start

if [ $? -eq 0 ]; then
    echo -e "${GREEN}[Success] ir Unblocker started successfully.${RESET}"
    echo -e "${YELLOW}[Info] You can access ir V4 via localhost.${RESET}"
else
    echo -e "${RED}[Error] Failed to start ir Unblocker. Please check the application logs for details.${RESET}"
    exit 1
fi
